...
const EXTRACTION_SCHEMA = {
  ...
  properties: {
    ...
    expectedDeliveryDate: {
      anyOf: [{ type: 'string' }, { type: 'null' }],
      description:
        'yyyy-MM-dd 형식. 정기구독·배송이면 "다음 배송일", "다음 결제일", "갱신일", "만료일", "다음 청구일" 중 가장 명확한 날짜를 최우선으로 추출. 일반 주문이면 예상 도착일. 명시되지 않으면 null.',
    },
    estimatedType: {
      anyOf: [{ type: 'string', enum: ['ELECTRONICS', 'ONLINE_ORDER', 'RECURRING_DELIVERY'] }, { type: 'null' }],
      description:
        '종류 추정. 아래 우선순위대로 판단해라.\n' +
        '1순위 RECURRING_DELIVERY: 메일에 아래 키워드/패턴 중 하나라도 있으면 반드시 이 값:\n' +
        '  - "정기배송", "정기결제", "정기구독", "구독", "구독 시작", "구독 갱신", "배송주기"\n' +
        '  - "매월", "매주", "격주", "4주마다", "N일마다", "다음 배송일"\n' +
        '  - "자동결제", "자동 갱신", "자동배송", "정기 할인"\n' +
        '  - "갱신일", "만료일", "다음 결제일", "다음 청구일", "구독 만료", "갱신 예정"\n' +
        '  - "월 구독", "연 구독", "도메인 갱신", "호스팅 갱신", "멤버십"\n' +
...
## 2단계: 종류 추정 (estimatedType) — 정기구독·배송을 최우선으로 인식
isOrderConfirmation=true일 때:

**RECURRING_DELIVERY (최우선)**: 다음 키워드/패턴 중 하나라도 있으면 반드시 이 값:
- "정기배송", "정기결제", "정기구독", "구독", "구독 시작", "구독 변경", "구독 갱신"
- "배송 주기", "다음 배송일", "N일마다", "매월", "매주", "격주", "4주마다", "2달마다"
- "자동결제", "자동 갱신", "자동배송", "정기 할인"
- "갱신일", "만료일", "다음 결제일", "다음 청구일", "구독 만료", "갱신 예정"
- "월 구독", "연 구독", "멤버십", "도메인 갱신", "호스팅 갱신"
- subscription, renewal, billing cycle, next billing date, auto-renew (영문 이메일)
...
- expectedDeliveryDate: 정기구독·배송이면 "다음 배송일", "다음 결제일", "갱신일", "만료일",
