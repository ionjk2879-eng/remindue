  ...
  properties: {
    ...
    expectedDeliveryDate: {
      ...
      description:
        ...
        '가장 명확한 날짜.\n' +
        'GENERAL(일반 구매)이면: 메일 본문에 사용자가 직접 적어 넣은 "예상 도착일"이 있으면 그 값을 ' +
        '무조건 최우선으로 쓴다(1회성 구매도 스토어 확인 메일에 정확한 도착 예정일이 없는 경우가 ' +
        '많다) — 이 날짜가 반품기한(7일)·A/S 보증(1년)의 기산일이 된다. 사용자가 적어 넣지 않았다면 ' +
        '스토어 본문의 예상 도착일로 대체.\n' +
        '명시되지 않으면 null.',
    },
    estimatedType: {
    ...
    intervalDays: {
      ...
      description:
        ...
        '"매월"/"한달마다"/"30일마다"=30, "N개월"/"N달"/"N개월마다"/"N달마다"=N*30(예: "4개월마다"=120, ' +
        '"5달마다"=150), "6주마다"=42, "2달마다"/"격월"=60, "분기마다"/"3달마다"=90, ' +
        '"연간"/"매년"/"1년마다"/"N년"/"N년마다"=N*365(예: "연간"/"매년"/"1년마다"=365, "2년마다"=730, ' +
        '"3년마다"=1095).\n' +
...
- "6주마다" → 42
- "N년" / "N년마다" / "연간" / "매년" / "1년마다" / annually / yearly → N*365 (예: "연간"/"매년"/
  "1년마다" → 365, "2년마다" → 730, "3년마다" → 1095) — 자동차 정기점검, 보험 갱신처럼 1년을
  넘어가는 주기도 이 규칙으로 처리한다.
...
- expectedDeliveryDate: RECURRING_DELIVERY면 사용자가 직접 적은 "첫 배송 예정일"/"예상 도착일"을
  ...
  결제일", "갱신일", "만료일", "다음 청구일" 중 가장 명확한 날짜.
  **GENERAL이면** 마찬가지로 사용자가 메일 본문에 직접 적어 넣은 "예상 도착일"이 있으면(주기 표기는
  필요 없다 — GENERAL은 1회성 구매라 "배송 주기" 자체가 의미 없다) 그 값을 스토어 본문의 도착
  예정일 표현보다 최우선으로 쓴다 — 이 날짜가 반품기한(7일)·A/S 보증(1년) 기산일이 되기 때문에
  중요하다. 예시: "예상 도착일: 8월 26일" → expectedDeliveryDate는 아래 날짜 표기 규칙대로 해석.
  없으면 스토어 본문의 예상 도착일로 대체.
